...
const fetchData = async () => {
  anchor_script.value = await dbManager.query(`select * from assistant_script where category_id=${props.liveid}`);
};
fetchData();
const handleDelete = async (item) => {
  try {
    await dbManager.execute("DELETE FROM assistant_script WHERE id = ?", [item.id]);
    // 删除后刷新数据
    fetchData();
  } catch (error) {
